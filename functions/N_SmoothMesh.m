function [faces, vertices] = N_SmoothMesh(faces, vertices, mode,itt,lambda,sigma)
%N_SmoothMesh performs all functions of PATCHSMOOTH described below.
%             This is just a modification wrapper to ensure compatibility
%             of input/output formats between ExportVoxelData scripts and
%             the smoothing function.
%
% Source of the PATCHSMOOTH function:
% http://www.mathworks.nl/matlabcentral/fileexchange/26710-smooth-triangulated-mesh
%
% INPUT:
%   faces    - matrix containing faces of the mesh surface
%   vertices - matrix of containing vertices of the surface
%
% See Inputs below for description of remaining inputs
%
% OUTPUT:
%   faces    - matrix containing faces of the smoothed mesh surface
%   vertices - matrix of containing vertices of the smoothed mesh surface
%
% PATCHSMOOTH FUNCTION DESCRIPTION
%--------------------------------------------------------------------------
...
%          original ratio in length between edges is preserved.
...
% Modification
% Convert input data to required structure
FV.vertices = vertices;
FV.faces    = faces;

if(nargin<3), mode =1; end
if(nargin<4), itt=1; end
...
% Commented as it is not required for this output format
% FV2=FV;
% FV2.vertices(:,1)=Vx;
% FV2.vertices(:,2)=Vy;
% FV2.vertices(:,3)=Vz;
