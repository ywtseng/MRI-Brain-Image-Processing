function [ mesh_list ] = N_PovCreateMeshList(mesh_name, color_criteria, color_compression, number_of_bins, transmit, texture_name)
%N_PovCreateMeshList outputs a mesh_list color coded structure that can be
%                    then exported to a PovRay scene
%
% INPUT:
...
%                       (Default = 100)
%   transmit          - scalar specifying transmittance of exported object
%                       (PovRay parameter) (Default = 0)
%   texture_name      - string with a name of a texture to give to exported
%                       objects (Default = '', i.e. no texture)
%
% OUTPUT:
...
% Check transmittance
if ~exist('transmit','var')
    transmit = 0;
end

% Check texture name 
if ~exist('texture_name','var')
    texture_name = '';
end
...
for i = 1 : length(color_criteria)
    ...
    % Texture
    mesh_list(i).texture = texture_name; 
